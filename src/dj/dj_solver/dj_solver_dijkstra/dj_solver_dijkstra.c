...
LOG_REGISTER("utils/dj/solver/dijkstra");
                              ...
                              bool *found_out)
{
    ...
    LOGD("Start of the Dijkstra solve algorithm (%u destinations)", (unsigned)goal_count);
    ...
    // List of path generated during the dijkstra algorithm
    ...
    dj_solver_common_heap_push(solver, 0, false, NULL, 0, NULL);
    ...
    do
    {
        ...
        if (current_node != NULL)
        {
            for (uint8_t adj_idx = 0; adj_idx < current_node->adj_count; adj_idx++)
            {
                ...
                if (node != NULL && !dj_graph_path_is_in_path(current_path, &node->pos))
                {
                    ...
                    if (index_in_list == -1)
                    {
                        ...
                        dj_solver_common_heap_push(solver, new_idx, false, NULL, 0, NULL);
                    ...
                    else
                    {
                            ...
                            dj_graph_path_get_duration(existing_path))
                        {
                            ...
                            // Re-insert into heap (lazy update: old entry stays, will be
                            // skipped when popped since path will be finished)
                            dj_solver_common_heap_push(
                                solver, (uint16_t)index_in_list, false, NULL, 0, NULL);
                            path_end_reached = false;
                        }
                    }
                }
        ...
        {
            dj_graph_path_t *candidate = dj_solver_common_heap_pop(solver, false, NULL, 0, NULL);
    ...
    LOGD("End of the Dijkstra solve algorithm (%u/%u goals found)",
